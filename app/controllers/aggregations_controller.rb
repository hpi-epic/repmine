class AggregationsController < ApplicationController
  ...
  def create
    ...
    unless params[:pattern_element_id].nil?
      alias_name ||= PatternElement.find(params[:pattern_element_id]).name
    end

    if aggregation.update_attributes(operation: params[:operation], alias_name: alias_name, distinct: params[:distinct])
